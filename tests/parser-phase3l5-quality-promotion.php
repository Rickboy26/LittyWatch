...
$recoverPrice=$ref->getMethod('recoverCanonicalPrice');
$recoverPrice->setAccessible(true);
$semantic=$ref->getMethod('semanticStatus');
...
$promotion=$recoverPrice->invoke($svc,$base+['raw_segment'=>'Warrior Tome 2e/ea']);
if (!is_array($promotion) || abs((float)$promotion['unit']-2.0)>0.00001 || ($promotion['basis']??null)!=='each') {
    fwrite(STDERR,"safe recovery did not promote canonical each basis\n"); exit(1);
}
[$status,$reason]=$semantic->invoke($svc,[
    'trade_type'=>'sell','item_key'=>'warrior-tome','price_amount'=>2.0,'price_currency'=>'e',
    'unit_price_ecto'=>$promotion['unit'],'price_basis'=>$promotion['basis'],'price_quality_reason'=>null,
]);
if ($status!=='trusted') { fwrite(STDERR,"promoted explicit each quote remained $status ($reason)\n"); exit(1); }

$unsafe=$recoverPrice->invoke($svc,['price_basis'=>'range','price_amount'=>675.0,'price_currency'=>'e','price_ecto'=>675.0,'quantity'=>null,'raw_segment'=>'Red Rock Candy Candy 225-675e']);
if ($unsafe!==null) { fwrite(STDERR,"range was incorrectly promoted\n"); exit(1); }

echo "Phase 3L.5 market-quality promotion OK\n";
