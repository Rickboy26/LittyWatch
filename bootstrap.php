...
function itemCatalog(): array {
    return [
        ...
        'Nicholas Set'=>['nick set','nickset'], 'Armbrace of Truth'=>['armbrace of truth','armbrace','armbraces','ambrace','ambraces','ambr'],
        ...
        'Droknar’s Key'=>["droknar's key",'droknars key'], 'Asterius Scythe'=>['asterius scythe'], 'Warrior Rune of Superior Vigor'=>['sup rune vigor','superior vigor'],
    ];
}
...
function extractPrice(string $segment): ?array {
    ...
    if(preg_match('/(?<![a-z0-9])([0-9]+(?:[.,][0-9]+)?)\s*(a|ambr(?:ace)?s?|armbraces?|e|ectos?|k|plat(?:inum)?)(?=\b|\/|$)/i',$segment,$m,PREG_OFFSET_CAPTURE)){
        ...
        $basis=preg_match('/^\s*\/\s*(?:ea|each)\b/i',$tail)?'each':(preg_match('/^\s*\/\s*(?:st|stk|stack)\b/i',$tail)?'stack':'total');
...
function detectQuantity(string $segment,?array $price): array {
    ...
    if(preg_match('/\b([0-9]+)\s+(?:gott|gotts|nickgifts?|gifts?|tomes?|unids?|rubies|sapphires|char carvings?|diessa chalices?|gaki|war horns?)\b/i',$segment,$m)) return [(float)$m[1],'total'];
    ...
    foreach(splitTypeBlocks($message) as $block){
        ...
        foreach($parts as $part){
            ...
            foreach($parsed as $p){
                ...
                $offers[]=['type'=>$p['type'],'item'=>$p['item'],'item_key'=>itemKey($p['item'].($p['details']?' '.$p['details']:'')),'details'=>$p['details'],'quantity'=>$qty,
...
function saveOffers(int $messageId,string $message): int {
    ...
    $ins=db()->prepare('INSERT OR IGNORE INTO offers(message_id,trade_type,item,item_key,details,quantity,price_amount,price_currency,price_ecto,unit_price_ecto,confidence,created_at,price_basis,raw_segment) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    $n=0;foreach(parseOffers($message) as $o){$ins->execute([$messageId,$o['type'],$o['item'],$o['item_key'],$o['details'],$o['quantity'],$o['amount'],$o['currency'],$o['ecto'],$o['unit_ecto'],$o['confidence'],date(DATE_ATOM),$o['basis'],$o['segment']]);$n+=$ins->rowCount();}return$n;
}

function httpGet(string $url): array {global$config;$headers=['User-Agent: LittyWatch/0.4 (+personal project)'];$ch=curl_init($url);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_FOLLOWLOCATION=>true,CURLOPT_TIMEOUT=>$config['request_timeout'],CURLOPT_HTTPHEADER=>$headers,CURLOPT_ENCODING=>'']);$body=curl_exec($ch);$err=curl_error($ch);$code=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);$type=(string)curl_getinfo($ch,CURLINFO_CONTENT_TYPE);curl_close($ch);if($body===false)throw new RuntimeException('cURL-fout: '.$err);return[$code,$type,$body];}
