...
final class StructuredOfferWriter {
 ...
 public function parseMessage(int $messageId,string $message,bool $replace=true):int{
  ...
  foreach($resolved as $r){
   ...
   $itemKeys[]=$r['item_key'];$ins->execute([$messageId,$r['trade_type'],$r['item'],$r['item_key'],$r['market_key'],$r['normalized_market_key'],$r['requirement'],$r['attribute_key'],$r['attribute_name'],$r['is_oldschool'],$r['is_inscribable'],$r['mods_json'],$r['relevant_json'],$r['profile_json'],$r['quantity'],$r['price_amount'],$r['price_currency'],$r['price_ecto'],$r['unit_price_ecto'],$r['price_basis'],$r['confidence'],$r['quality_status'],$r['quality_reason'],$r['raw_segment'],'v5.2-phase7d-live-dedup',date(DATE_ATOM),$r['quality_status']==='accepted'?'active':'rejected',date(DATE_ATOM),$r['exchange_item'],$r['exchange_item_key'],$r['exchange_give_quantity'],$r['exchange_receive_quantity']]);$n+=$ins->rowCount();}}
  if($this->lifecycle!==null){
   $this->lifecycle->rebuild($messageId);
   // LITTYWATCH_PHASE7D_LIVE_DEDUP
   // A repeated Kamadan advertisement receives a new message_id. Keep the new
   // observation active, but supersede older active listings from the same
   // player for the same trade direction and concrete normalized market variant.
   // Historical rows are preserved; only lifecycle state changes.
   $this->supersedeRepeatedLiveOffers($messageId);
   (new MarketQualityService($this->pdo))->rebuildForItemKeys($itemKeys);
  }
  return $n;
 }

 private function supersedeRepeatedLiveOffers(int $messageId):void{
  $now=date(DATE_ATOM);

  // Only accepted/active offers in the current message participate. We join
  // messages for player identity; normalized_market_key keeps real variants
  // separate (q9 vs q10, attribute variants, OS/inscribable variants, etc.).
  $sql="
UPDATE structured_offers AS old
SET
    lifecycle_status='superseded',
    superseded_by=(
        SELECT cur.id
        FROM structured_offers AS cur
        JOIN messages AS cm ON cm.id=cur.message_id
        JOIN messages AS om ON om.id=old.message_id
        WHERE cur.message_id=:message_id
          AND cur.lifecycle_status='active'
          AND cur.quality_status='accepted'
          AND old.lifecycle_status='active'
          AND old.quality_status='accepted'
          AND old.message_id<>cur.message_id
          AND lower(trim(COALESCE(om.player,'')))=lower(trim(COALESCE(cm.player,'')))
          AND COALESCE(old.trade_type,'')=COALESCE(cur.trade_type,'')
          AND COALESCE(NULLIF(old.normalized_market_key,''),old.item_key,'')=
              COALESCE(NULLIF(cur.normalized_market_key,''),cur.item_key,'')
...
WHERE old.lifecycle_status='active'
  AND old.quality_status='accepted'
  AND old.message_id<>:message_id2
  AND EXISTS (
      SELECT 1
      FROM structured_offers AS cur
      JOIN messages AS cm ON cm.id=cur.message_id
      JOIN messages AS om ON om.id=old.message_id
      WHERE cur.message_id=:message_id3
        AND cur.lifecycle_status='active'
        AND cur.quality_status='accepted'
        AND lower(trim(COALESCE(om.player,'')))=lower(trim(COALESCE(cm.player,'')))
        AND COALESCE(old.trade_type,'')=COALESCE(cur.trade_type,'')
