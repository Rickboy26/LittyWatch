...
</style><link rel="stylesheet" href="/assets/v2/platform.css?v=310">
</head><body><main class="wrap"><div class="top"><div><h1>Markttrends</h1><div class="muted">V2.1 — vergelijking van eerste en laatste snapshot binnen 7 dagen.</div></div><nav><a href="/">Dashboard</a> · <a href="/watchlist">Watchlist</a> · <a href="/admin/snapshot">Snapshot maken</a></nav></div><section class="panel"><table><thead><tr><th>Markt</th><th>Mediaan WTB</th><th>Mediaan WTS</th><th>WTS-beweging</th><th>Offers</th><th>Traders</th><th>Snapshot</th></tr></thead><tbody><?php if(!$rows): ?><tr><td colspan="7" class="muted">Nog onvoldoende snapshots. Maak eerst een snapshot en herhaal dit later.</td></tr><?php endif; ?><?php foreach($rows as $row): $change=$row['wts_change_pct']; ?><tr><td><a href="/market?key=<?= urlencode((string)$row['market_key']) ?>"><?= e((string)$row['market_key']) ?></a></td><td><?= p($row['median_wtb_ecto']) ?></td><td><?= p($row['median_wts_ecto']) ?></td><td class="<?= $change===null?'':((float)$change>=0?'up':'down') ?>"><?= $change===null?'—':number_format((float)$change,1,',','.').'%' ?></td><td><?= (int)$row['active_offers'] ?></td><td><?= (int)$row['unique_traders'] ?></td><td class="muted"><?= e((string)$row['captured_at']) ?></td></tr><?php endforeach; ?></tbody></table></section></main><script src="/assets/v2/platform.js?v=310"></script>
</body></html>
