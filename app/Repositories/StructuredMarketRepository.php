...
final class StructuredMarketRepository{
 ...
 public function analytics(string $key):array{
  ...
  $s=$this->pdo->prepare("SELECT so.trade_type,so.unit_price_ecto,so.price_basis,so.quantity,m.player,m.posted_at FROM structured_offers so JOIN messages m ON m.id=so.message_id WHERE $market=:k AND so.quality_status='accepted' AND so.lifecycle_status='active' AND so.unit_price_ecto IS NOT NULL AND COALESCE(so.price_basis,'') NOT IN ('bundle','currency_exchange','unknown','currency_conversion','unqualified','uncertain','range') ORDER BY m.id ASC,so.id ASC");
