...
return static function(Router $router,Container $c):void{
 ...
 $router->get('/game-assets',fn($r)=>$c->get(AssetController::class)->index($r));$router->post('/game-assets',fn($r)=>$c->get(AssetController::class)->update($r));$router->post('/game-assets/auto-link',fn($r)=>$c->get(AssetController::class)->autoLink($r));$router->post('/admin/assets-knowledge-cleanup',fn($r)=>$c->get(AssetController::class)->cleanupKnowledge($r));$router->post('/admin/assets-gwca-ids',fn($r)=>$c->get(AssetController::class)->importGwcaIds($r));$router->post('/admin/assets-runtime-ids',fn($r)=>$c->get(AssetController::class)->importRuntimeIds($r));$router->get('/game-assets/wiki-icon',fn($r)=>$c->get(AssetController::class)->wikiIcon($r));$router->get('/system',fn($r)=>$c->get(SystemController::class)->index($r));
