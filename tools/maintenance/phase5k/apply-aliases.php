...
$files=glob(dirname(__DIR__,3).'/data/exports/littywatch-phase5k-alias-dryrun-*.json');
if(!$files){fwrite(STDERR,"Geen 5K dry-run gevonden.\n");exit(1);}
rsort($files);$path=$files[0];
$data=json_decode((string)file_get_contents($path),true);
if(!is_array($data)){fwrite(STDERR,"Ongeldig 5K rapport.\n");exit(1);}
...
foreach($data as $r){
    ...
    $ins->execute([
        ...
        'phase5k_trusted_pattern',$r['group_id'],0.99,1,
        'Phase 5K deterministic leftover alias',$now,$now
    ]);
    $count++;
}
echo "Phase 5K aliases geactiveerd: {$count}\n";
