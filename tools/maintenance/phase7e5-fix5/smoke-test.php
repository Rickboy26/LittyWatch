...
$fail=0;
echo "Phase 7E.5 FIX5 smoke-test\n";
...
foreach([
    ...
    foreach($p->parse($msg) as $o){
        ...
        $name=mb_strtolower(trim($o->item));
        if(preg_match('/^miniature\s+ghostly\s+hero$/iu',$name))$collision=true;
    }
    printf("%-32s => collision=%-3s parsed=[%s] %s\n",
        $msg,
        $collision?'YES':'NO',
        implode(', ',$items),
        !$collision?'OK':'FAIL'
...
if($fail){
    fwrite(STDERR,"\nPhase 7E.5 FIX5 smoke-test: FAIL ({$fail})\n");
    exit(1);
}
echo "\nPhase 7E.5 FIX5 smoke-test: OK\n";
