...
$fail=0;
echo "Phase 7E.5 FIX3 smoke-test\n";
...
foreach($p->parse($msg) as $o){
    ...
    if(strcasecmp($o->item,"Hero's Strongbox")===0)$strongbox=true;
}

$ok=!$collision;
printf(
    "%-24s => collision=%-3s strongbox=%-3s parsed=[%s] %s\n",
...
if($fail){
    fwrite(STDERR,"\nPhase 7E.5 FIX3 smoke-test: FAIL ({$fail})\n");
    exit(1);
}
echo "\nPhase 7E.5 FIX3 smoke-test: OK\n";
