...
$files=glob(dirname(__DIR__,3).'/data/exports/littywatch-phase5g-alias-dryrun-*.json');
if(!$files){fwrite(STDERR,"Geen 5G dry-run gevonden.\n");exit(1);}
rsort($files);$path=$files[0];

$data=json_decode((string)file_get_contents($path),true);
if(!is_array($data)){fwrite(STDERR,"Ongeldig 5G dry-run bestand.\n");exit(1);}
...
foreach($data as $r){
    ...
    $ins->execute([
        ...
        'phase5g_trusted_pattern',$r['group_id'],0.99,1,
        'Phase 5G trusted typo/material/unique alias',$now,$now
    ]);
    $count++;
}
echo "Phase 5G aliases geactiveerd: {$count}\n";
