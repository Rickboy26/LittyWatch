...
$files=glob(dirname(__DIR__,3).'/data/exports/littywatch-phase5h-alias-dryrun-*.json');
if(!$files){fwrite(STDERR,"Geen 5H dry-run gevonden.\n");exit(1);}
rsort($files);$path=$files[0];
$data=json_decode((string)file_get_contents($path),true);
if(!is_array($data)){fwrite(STDERR,"Ongeldig 5H rapport.\n");exit(1);}
...
foreach($data as $r){
    ...
    $ins->execute([
        ...
        'phase5h_trusted_pattern',$r['group_id'],0.99,1,
        'Phase 5H trusted currency/material/typo alias',$now,$now
    ]);
    $count++;
}
echo "Phase 5H aliases geactiveerd: {$count}\n";
