...
$fail=0;
echo "Phase 7E.5 FIX6 smoke-test\n";
...
foreach($tests as [$msg,$want,$wantDed]){
    ...
    foreach($p->parse($msg) as $o){
        if(strcasecmp(trim($o->item),$want)===0){
            $found=$o;
            break;
        }
    }
...
if($fail){
    fwrite(STDERR,"\nPhase 7E.5 FIX6 smoke-test: FAIL ({$fail})\n");
    exit(1);
}

echo "\nPhase 7E.5 FIX6 smoke-test: OK\n";
